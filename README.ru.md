...
(на основе `0.0.3`). Что добавлено/изменено относительно апстрима:
...
- **10 фиксов корректности/надёжности** на стороне выходной ноды, найденных
  ревью кода, каждый с регрессионным тестом: две паники, вызываемые удалённой
  стороной и валившие весь процесс, decompression-bomb DoS в legacy-кодеке,
  тихо игнорируемый `--local-ip`, RST от клиента, не доходящий до реального
  сервера, неограниченная утечка памяти, тихо проглатываемые ошибки отправки
  транспорта, блокировка всей обработки пакетов на время periodic-скана
  conntrack, отсутствие graceful shutdown по SIGTERM.
