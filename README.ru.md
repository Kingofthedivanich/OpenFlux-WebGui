...
С настроенным токеном у поля URL для yandex/vyandex в «Добавить клиента»
появляется кнопка **Сгенерировать**: она создаёт пустой документ на Диске
этого Яндекс-аккаунта, публикует его и подставляет публичную ссылку — не
нужно создавать документ вручную в браузере и копировать ссылку. Нажми
ещё раз — добавится ещё один документ этому же клиенту (multi-stream).
...
разреши доступ и скопируй `access_token` из адресной строки после
редиректа в `--yandex-token-file`. Это тот же официальный Disk API, которым
